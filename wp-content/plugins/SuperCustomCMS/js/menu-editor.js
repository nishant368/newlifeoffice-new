...
function outputWpMenu(menu){
	...
	$('#supercustomcms_menu_box').empty();
	$('#supercustomcms_submenu_box').empty();
	...
	$('#supercustomcms_menu_box .supercustomcms_menu:first').click();
}
...
function outputTopMenu(menu, afterNode){
	...
	//Display
	submenu.appendTo('#supercustomcms_submenu_box');
	if ( typeof afterNode != 'undefined' ){
		$(afterNode).after(menu_obj);
	} else {
		menu_obj.appendTo('#supercustomcms_menu_box');
	}
...
function buildTopMenu(menu){
	var subclass = '';
	if ( menu.separator ) {
		subclass = subclass + ' supercustomcms_menu_separator';
	}
	
	//Create the menu HTML
	var menu_obj = $('<div></div>')
		.attr('class', "supercustomcms_container supercustomcms_menu "+subclass)
		.attr('id', 'supercustomcms-topmenu-'+(wsIdCounter++))
	...
	var contents = [];
	contents.push(
		'<div class="supercustomcms_item_head">',
			menu.separator ? '' : '<a class="supercustomcms_edit_link"> </a><div class="supercustomcms_flag_container"> </div>',
			'<span class="supercustomcms_item_title">',
				((menu.menu_title!=null) ? menu.menu_title : menu.defaults.menu_title),
			'&nbsp;</span>',
		'</div>',
		'<div class="supercustomcms_editbox" style="display: none;"></div>'
	);
	...
	if ( !menu.separator ){
		...
		menu_obj.droppable({
			'hoverClass' : 'supercustomcms_menu_drop_hover',
			
			'accept' : (function(thing){
				return thing.hasClass('supercustomcms_item');
			}),
...
function buildSubmenu(items){
	//Create a container for menu items, even if there are none
	var submenu = $('<div class="supercustomcms_submenu"style="display:none;"></div>');
	submenu.attr('id', 'supercustomcms-submenu-'+(wsIdCounter++));
...
function buildMenuItem(entry){
	...
	var item = $('<div class="supercustomcms_container supercustomcms_item">')
	...
	contents.push(
		'<div class="supercustomcms_item_head">',
			'<a class="supercustomcms_edit_link"> </a><div class="supercustomcms_flag_container"> </div>',
			'<span class="supercustomcms_item_title">',
...
var knownMenuFields = {
	...
	'file' : {
		...
		type : 'text',
		defaultValue: '',
		addDropdown : 'supercustomcms_page_selector',
		visible: true
	},
	'page_title' : {
...
function buildEditboxFields(containerNode, entry){
    ...
    var basicFields = $('<div class="supercustomcms_edit_panel supercustomcms_basic"></div>').appendTo(containerNode);
    var advancedFields = $('<div class="supercustomcms_edit_panel supercustomcms_advanced"></div>').appendTo(containerNode);
	...
	containerNode.append(
		'<div class="supercustomcms_toggle_container"><a href="#" class="supercustomcms_toggle_advanced_fields"'+
...
function buildEditboxField(entry, field_name, field_settings){
	...
	switch(field_settings.type){
		case 'select':
			inputBox = $('<select class="supercustomcms_field_value">');
			var option = null;
        ...
        case 'checkbox':
            inputBox = $('<label><input type="checkbox"'+(value?' checked="checked"':'')+ ' class="supercustomcms_field_value"> '+
		...
		case 'text':
		default:
			inputBox = $('<input type="text" class="supercustomcms_field_value">').val(value);
	}
	
	
	var className = "supercustomcms_edit_field supercustomcms_edit_field-"+field_name;
	if(entry[field_name]==null){
		className += ' supercustomcms_input_default';
	}
	
	var hasDropdown = (typeof(field_settings['addDropdown']) != 'undefined') && field_settings.addDropdown;
	if ( hasDropdown ){
		className += ' supercustomcms_has_dropdown';
	}
	...
	if ( hasDropdown ){
		...
		var dropdownId = 'supercustomcms_cap_selector';
		if ( typeof(field_settings.addDropdown) == 'string' ){
			dropdownId = field_settings.addDropdown;
		}
		editField.append(
			$('<input type="button" value="&#9660;">')
				.addClass('button supercustomcms_dropdown_button')
				.attr('tabindex', '-1')
	...
	editField
		.append('<img src="'+imagesUrl+'/transparent16.png" class="supercustomcms_reset_button" title="Reset to default value">&nbsp;</img>')
...
function makeBoxSortable(menuBox){
	//Make the submenu sortable
	menuBox.sortable({
		items: '> .supercustomcms_container',
		cursor: 'move',
		dropOnEmpty: true,
		cancel : '.supercustomcms_editbox, .supercustomcms_edit_link'
	});
}
...
function readMenuTreeState(){
	...
	$('#supercustomcms_menu_box .supercustomcms_menu').each(function(i) {
 ...
 *
 * Inputs : 
 * 	menu_div - DOM node, typically one with the .supercustomcms_menu class.
...
function readMenuState(menu_div, position){
	...
	menu.separator = menu_div.hasClass('supercustomcms_menu_separator');
	menu.hidden = menu_div.hasClass('supercustomcms_hidden'); 

	//Gather the menu's items, if any
	menu.items = {};
	var item_position = 0;
	$('#'+menu_div.data('submenu_id')).find('.supercustomcms_item').each(function (i) {
 ...
 *
 * Inputs : 
 *	item_div - DOM node containing the editor widget, usually with the .supercustomcms_item class.  
...
function readItemState(item_div, position){
	...
	item.hidden = item_div.hasClass('supercustomcms_hidden');
...
function readAllFields(container){
	if ( !container.hasClass('supercustomcms_container') ){
		container = container.parents('supercustomcms_container').first();
	}
	...
	container.find('.supercustomcms_edit_field').each(function() {
		var field = $(this);
		...
		input_box = field.find('.supercustomcms_field_value');
		
		//Save null if default used, custom value otherwise
		if (field.hasClass('supercustomcms_input_default')){
...
function addMenuFlag(item, flag){
	...
	var item_class = 'supercustomcms_' + flag;
	var img_class = 'supercustomcms_' + flag + '_flag';
	
	item.addClass(item_class);
	//Add the flag image
	var flag_container = item.find('.supercustomcms_flag_container');
	if ( flag_container.find('.' + img_class).length == 0 ){
		flag_container.append('<div class="supercustomcms_flag '+img_class+'" title="'+item_flags[flag]+'"></div>');
	}
}
...
function removeMenuFlag(item, flag){
	...
	item.removeClass('supercustomcms_' + flag);
...
function menuHasFlag(item, flag){
	return $(item).hasClass('supercustomcms_'+flag);
}

function clearMenuFlags(item){
	item = $(item);
	item.find('.supercustomcms_flag').remove();
...
var supercustomcms_paste_count = 0;
...
$(document).ready(function(){
    ...
    var mainMenuBox = $('#supercustomcms_menu_box');
    ...
    $('#supercustomcms_menu_editor .supercustomcms_container').live('click', (function () {
		var container = $(this);
		if ( container.hasClass('supercustomcms_active') ){
			return;
		}
		
		//Highlight the active item and un-highlight the previous one
		container.addClass('supercustomcms_active')
		container.siblings('.supercustomcms_active').removeClass('supercustomcms_active');
		if ( container.hasClass('supercustomcms_menu') ){
    ...
    $('#supercustomcms_menu_editor .supercustomcms_edit_link').live('click', (function () {
    	var container = $(this).parents('.supercustomcms_container').first(); 
		var box = container.find('.supercustomcms_editbox');
		...
		$(this).toggleClass('supercustomcms_edit_link_expanded');
		//show/hide the editbox
		if ($(this).hasClass('supercustomcms_edit_link_expanded')){
			box.show();
		} else {
    ...
    $('#supercustomcms_menu_editor .supercustomcms_reset_button').live('click', (function () {
        //Find the field div (it holds the default value)
        var field = $(this).parent();
    	//Find the related input field
		var input = field.find('.supercustomcms_field_value');
		if ( (input.length > 0) && (field.length > 0) ) {
			...
			field.addClass('supercustomcms_input_default');
    ...
    function fieldValueChange(){
        var input = $(this);
		var field = input.parents('.supercustomcms_edit_field').first();
		...
		if ( field.data('default_value') != value ) {
			field.removeClass('supercustomcms_input_default');
		}
		
        var fieldName = field.data('field_name');
		if ( fieldName == 'menu_title' ){
            //If the changed field is the menu title, update the header
			field.parents('.supercustomcms_container').first().find('.supercustomcms_item_title').html(input.val()+'&nbsp;');
		} else if ( fieldName == 'custom' ){
            //Show/hide the custom flag
            var myContainer = field.parents('.supercustomcms_container').first();
            if ( value ){
			...
			if ( value == '' ){
				field.find('.supercustomcms_reset_button').click();
			}
		}
    }
	$('#supercustomcms_menu_editor .supercustomcms_field_value').live('click', fieldValueChange);
	...
	$('#supercustomcms_menu_editor').change(function(event){
		var target = $(event.target);
		if ( target.is('.supercustomcms_field_value') ){
			fieldValueChange.call(target, event);
		}
	});
	
	//Show/hide advanced fields
	$('#supercustomcms_menu_editor .supercustomcms_toggle_advanced_fields').live('click', function(){
		var self = $(this);
		var advancedFields = self.parents('.supercustomcms_container').first().find('.supercustomcms_advanced');
	...
	var availableDropdowns = {
		'supercustomcms_cap_selector' : {
			list : $('#supercustomcms_cap_selector'),
			currentOwner : null,
			timeoutForgetOwner : 0
		},
		'supercustomcms_page_selector' : {
			list : $('#supercustomcms_page_selector'),
			currentOwner : null,
			timeoutForgetOwner : 0
		}
	};
	...
	$('#supercustomcms_menu_editor input.supercustomcms_dropdown_button').live('click',function(event){
		var button = $(this);
		var inputBox = button.parent().find('input.supercustomcms_field_value');
	...
	$('#supercustomcms_menu_editor .supercustomcms_has_dropdown input.supercustomcms_field_value').live('keyup', function(event){
		if ( event.which == 40 ){
			$(this).parent().find('input.supercustomcms_dropdown_button').click();
		}
	});
	
	//Event handlers for the dropdowns themselves
	var dropdownNodes = $('.supercustomcms_dropdown');
	...
	dropdownNodes.keydown(function(event){
		...
		if ( event.which == 27 ){
			var inputBox = $(dropdown.currentOwner).parent().find('input.supercustomcms_field_value');
			
			dropdown.list.hide();
			if ( dropdown.currentOwner ){
				$(dropdown.currentOwner).parent().find('input.supercustomcms_field_value').focus();
			}
			dropdown.currentOwner = null;
	 ...
	 *************************************************************************/
	//Show/Hide menu
	$('#supercustomcms_hide_menu').click(function () {
		//Get the selected menu
		var selection = $('#supercustomcms_menu_box .supercustomcms_active');
		if (!selection.length) return;
		
		//Mark the menu as hidden/visible
		//selection.toggleClass('supercustomcms_hidden');
		toggleMenuFlag(selection, 'hidden');
		
		//Also mark all of it's submenus as hidden/visible
		if ( menuHasFlag(selection,'hidden') ){
			$('#' + selection.data('submenu_id') + ' .supercustomcms_item').each(function(){
	...
	//Delete menu
	$('#supercustomcms_delete_menu').click(function () {
		//Get the selected menu
		var selection = $('#supercustomcms_menu_box .supercustomcms_active');
		if (!selection.length) return;
	...
	//Copy menu
	$('#supercustomcms_copy_menu').click(function () {
		//Get the selected menu
		var selection = $('#supercustomcms_menu_box .supercustomcms_active');
		if (!selection.length) return;
	...
	//Cut menu
	$('#supercustomcms_cut_menu').click(function () {
		//Get the selected menu
		var selection = $('#supercustomcms_menu_box .supercustomcms_active');
		if (!selection.length) return;
	...
	//Paste menu
	$('#supercustomcms_paste_menu').click(function () {
	...
	//New menu
	$('#supercustomcms_new_menu').click(function () {
		supercustomcms_paste_count++;
		...
		var menu = {
			...
			items : {}, //No items
			defaults : {
				menu_title : 'Custom Menu '+supercustomcms_paste_count,
				page_title : '',
				access_level : 'read',
				file : randomId,
				css_class : 'menu-top',
		...
		result.menu.find('.supercustomcms_edit_link').click();
	});
	
	//New separator
	$('#supercustomcms_new_separator').click(function () {
		supercustomcms_paste_count++;
	 ...
	 *************************************************************************/
	//Show/Hide item
	$('#supercustomcms_hide_item').click(function () {
		//Get the selected item
		var selection = $('#supercustomcms_submenu_box .supercustomcms_submenu:visible .supercustomcms_active');
		if (!selection.length) return;
	...
	//Delete menu
	$('#supercustomcms_delete_item').click(function () {
		//Get the selected menu
		var selection = $('#supercustomcms_submenu_box .supercustomcms_submenu:visible .supercustomcms_active');
		if (!selection.length) return;
	...
	//Copy item
	$('#supercustomcms_copy_item').click(function () {
		//Get the selected item
		var selection = $('#supercustomcms_submenu_box .supercustomcms_submenu:visible .supercustomcms_active');
		if (!selection.length) return;
	...
	//Cut item
	$('#supercustomcms_cut_item').click(function () {
		//Get the selected item
		var selection = $('#supercustomcms_submenu_box .supercustomcms_submenu:visible .supercustomcms_active');
		if (!selection.length) return;
	...
	//Paste item
	$('#supercustomcms_paste_item').click(function () {
		//Check if anything has been copied/cut
		...
		if (selection.length > 0) {
			...
			//Otherwise add the pasted item at the end
			$('#supercustomcms_submenu_box .supercustomcms_submenu:visible').append(new_item); 
		}
	...
	//New item
	$('#supercustomcms_new_item').click(function () {
		if ($('.supercustomcms_submenu:visible').length < 1) { 
		...
		var entry = {
			...
			open_in : null,
			defaults : {
				menu_title : 'Custom Item ' + supercustomcms_paste_count,
				page_title : '',
				access_level : 'read',
		...
		$('#supercustomcms_submenu_box .supercustomcms_submenu:visible').append(menu);
		
		//The items's editbox is always open
		menu.find('.supercustomcms_edit_link').click();
	});
	...
	function compareMenus(a, b){
		...
		var aTitle = jsTrim( $(a).find('.supercustomcms_item_title').text() );
		var bTitle = jsTrim( $(b).find('.supercustomcms_item_title').text() );
	...
	$('#supercustomcms_sort_ascending').click(function () {
		var submenu = $('#supercustomcms_submenu_box .supercustomcms_submenu:visible');
		if (submenu.length < 1) { 
			return; //Abort if no submenu visible 
		}
		
		submenu.find('.supercustomcms_container').sort(compareMenus);
	});
	
	//Sort items in descending order
	$('#supercustomcms_sort_descending').click(function () {
		var submenu = $('#supercustomcms_submenu_box .supercustomcms_submenu:visible');
		if (submenu.length < 1) { 
			return; //Abort if no submenu visible 
		}
		
		submenu.find('.supercustomcms_container').sort((function(a, b){
	...
	$('#supercustomcms_save_menu').click(function () {
		var data = encodeMenuAsJSON();
		$('#supercustomcms_data').val(data);
		$('#supercustomcms_main_form').submit();
	});
	
	//Load default menu - load the default WordPress menu
	$('#supercustomcms_load_menu').click(function () {
	...
	$('#supercustomcms_reset_menu').click(function () {
	...
	$('#supercustomcms_export_menu').click(function(){
		var button = $(this);
	...
	$('#supercustomcms_cancel_export').click(function(){
		$('#export_dialog').dialog('close');
	});
	...
	$('#supercustomcms_cancel_import').click(function(){
		$('#import_dialog').dialog('close');
	});
	
	$('#supercustomcms_import_menu').click(function(){
		...
		$('#supercustomcms_start_import').attr('disabled', 'disabled'); 
	...
	$('#import_file_selector').change(function(){
		if ( $(this).val() ){
			$('#supercustomcms_start_import').removeAttr('disabled');
		} else {
			$('#supercustomcms_start_import').attr('disabled', 'disabled');
		};
	});
...
jQuery(function($){
	var screenOptions = $('#supercustomcms-ame-screen-meta-contents');
	var checkbox = screenOptions.find('#supercustomcms-hide-advanced-settings');
	...
	checkbox.click(function(){
		...
		if ( hideAdvancedSettings ){
			$('#supercustomcms_menu_editor div.supercustomcms_advanced').hide();
			$('#supercustomcms_menu_editor a.supercustomcms_toggle_advanced_fields').text(captionShowAdvanced).show();
		} else {
			$('#supercustomcms_menu_editor div.supercustomcms_advanced').show();
			$('#supercustomcms_menu_editor a.supercustomcms_toggle_advanced_fields').text(captionHideAdvanced).hide();
		}
		
		$.post(
			adminAjaxUrl,
			{
				'action' : 'supercustomcms_ame_save_screen_options',
