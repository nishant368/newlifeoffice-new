...
?>
		...
		<div class="supercustom_cms_section">
                ...
                <h3 class="hndle" id="config_edit_post"><img src="<?php _e(WP_SUPERCUSTOM_CMS_ADDRESS); ?>images/trans.png" class="inactive" alt=""/><?php _e('Write options - Post', FB_SUPERCUSTOM_CMS_TEXTDOMAIN ); ?></h3>                                                                                
				...
				<div class="clearfix"></div></div>
					...
					<table summary="config_edit_post" class="widefat">
						<thead>
							<tr>
								<th><?php _e('Write options - Post', FB_SUPERCUSTOM_CMS_TEXTDOMAIN ); ?></th>
								<?php
						...
						<?php
							...
							$metaboxes_names = array(
								...
								__('Excerpt', FB_SUPERCUSTOM_CMS_TEXTDOMAIN ),
								__('Trackbacks', FB_SUPERCUSTOM_CMS_TEXTDOMAIN ),
								__('Custom Fields'),
								__('Comments', FB_SUPERCUSTOM_CMS_TEXTDOMAIN ),
							...
							$__wp_supercustom_cms_own_post_values  = __wp_supercustom_cms_get_option_value('__wp_supercustom_cms_own_post_values');
							$__wp_supercustom_cms_own_post_values = preg_split( "/\r\n/", $__wp_supercustom_cms_own_post_values );
							foreach ( (array) $__wp_supercustom_cms_own_post_values as $key => $__wp_supercustom_cms_own_post_value ) {
								$__wp_supercustom_cms_own_post_value = trim($__wp_supercustom_cms_own_post_value);
								array_push($metaboxes, $__wp_supercustom_cms_own_post_value);
							}
							
							$__wp_supercustom_cms_own_post_options = __wp_supercustom_cms_get_option_value('__wp_supercustom_cms_own_post_options');
							$__wp_supercustom_cms_own_post_options = preg_split( "/\r\n/", $__wp_supercustom_cms_own_post_options );
							foreach ( (array) $__wp_supercustom_cms_own_post_options as $key => $__wp_supercustom_cms_own_post_option ) {
								$__wp_supercustom_cms_own_post_option = trim($__wp_supercustom_cms_own_post_option);
								array_push($metaboxes_names, $__wp_supercustom_cms_own_post_option);
							}
							...
							foreach ($metaboxes as $index => $metabox) {
								if ($metabox != '') {
									...
									foreach ($user_roles as $role) {
										echo '<td class="num"><input id="check_post'. $role.$x .'" type="checkbox"' . $checked_user_role_[$role] . ' name="wp_supercustom_cms_disabled_metaboxes_post_'. $role .'_items[]" value="' . $metabox . '" /></td>' . "\n";
									}
									echo '</tr>' . "\n";
									$x++;
								}
					...
					<table summary="config_own_post" class="widefat">
						<thead>
							<tr>
								<th><?php _e('Your own post options', FB_SUPERCUSTOM_CMS_TEXTDOMAIN ); echo '<br />'; _e('ID or class', FB_SUPERCUSTOM_CMS_TEXTDOMAIN ); ?></th>
						...
						<tbody>
							...
							<tr valign="top">
								<td>
									<textarea name="__wp_supercustom_cms_own_post_options" cols="60" rows="3" id="__wp_supercustom_cms_own_post_options" style="width: 95%;" ><?php echo __wp_supercustom_cms_get_option_value('__wp_supercustom_cms_own_post_options'); ?></textarea>
									<br />
								...
								<td>
									<textarea class="code" name="__wp_supercustom_cms_own_post_values" cols="60" rows="3" id="__wp_supercustom_cms_own_post_values" style="width: 95%;" ><?php echo __wp_supercustom_cms_get_option_value('__wp_supercustom_cms_own_post_values'); ?></textarea>
									<br />
