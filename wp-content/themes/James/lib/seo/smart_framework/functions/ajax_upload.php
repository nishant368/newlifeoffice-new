<?php
add_action('admin_head', 'smartSeo_ajaxuploader');
function smartSeo_ajaxuploader() {
	global $post;
	
	$plugin_uri =  home_url() . '/wp-content/plugins/' . PLUGIN_NAME . '/';
?>
			  ...
			  onComplete: function(file, response) {
				...
				// If there was an error
			  	if(response.error ){
					...
					if(isSlider != 'upload') { 
						var buildReturn = '<img id="image_'+clickedID+'" src="<?php echo $plugin_uri . '/thumb.php?src=' ?>'+response.url+'&w=120" />';
					...
					if(isSlider == 'upload') 
					{
						...
						ImgDetails += '<img src="<?php echo $plugin_uri; ?>/thumb.php?src='+response.url+'&w=40" target="_blank" alt="" /><br/><br/><img src="<?php echo ADMIN_IMAGES ?>/drag.png" alt="Drag" title="Drag and Move this item" />';
						ImgDetails += '</td><td valign="top">';
						ImgDetails += '<div>' + file + '<div style="float:right; width:20px;"><a href="#" class="remove" onclick="return false" ><img src="<?php echo ADMIN_IMAGES ?>/bin.gif" alt="Remove" title="Remove" /></a></div></div>';
